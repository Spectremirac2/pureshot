// 1vDOQUZ Arena 2.0 — mini harita (canvas). Statik zemin bir kez çizilir; birimler ~8 fps güncellenir.

import { ARENA_R, RIVER_HALF, ISLAND_R, riverPoint, FOUNTAIN, ROSHAN_PIT, TREES, GATE_ANGLES } from './map.js';
...
export function createMinimap(canvas) {
  ...
  function draw(game, dt, force = false) {
    ...
    for (const p of game.pickups) {
      if (p.kind !== 'aegis' && p.kind !== 'cheese' && p.kind !== 'rapierItem') continue;
      g.fillStyle = p.kind === 'rapierItem' ? '#ff4a5a' : '#f6d98a';
      g.beginPath();
    ...
    for (const e of game.foes) {
      ...
      g.fillStyle = e.kind === 'creep' ? '#c24a3a' : e.thief ? '#ffd84a' : '#ff3b4b';
      const s = (e.kind === 'creep' ? 2.2 : 3) * k;
      g.beginPath();
